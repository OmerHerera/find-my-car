    ...
    <main className='app-shell'>
      ...
      {panel?.kind === 'add' && (
        <AddDialog
          ...
          onClose={() => setPanel(null)}
          onSave={async (input) => {
            setCars(await addData(cars, input));
            setPanel(null);
          }}
        />
      )}
      {panel?.kind === 'edit' && (
        <CarFormDialog
          ...
          onClose={() => setPanel(null)}
          onSave={async (input) => {
            setCars(await updateData(cars, panel.car.id, input));
            setPanel(null);
          }}
        />
      )}
      {panel?.kind === 'remove' && (
        <RemoveDialog
          ...
          onClose={() => setPanel(null)}
          onRemove={async () => {
            setCars(await removeData(cars, panel.car.id));
            setPanel(null);
          }}
        />
      )}
  ...
  return (
    <article className={`car-card ${featured ? 'featured' : ''}`}>
      <div className='card-heading'>
        ...
        <span className={`status ${car.parking ? '' : 'unknown'}`}>
          <i />
